...
class Themesflat_Recent_Post extends WP_Widget {
    ...
    function __construct() {
        ...
        parent::__construct(
            'widget_recent_post',
            esc_html__( 'Themesflat - Relatest Post', 'themesflat' ),
            array(
                'classname'   => 'widget-recent-news',
                'description' => esc_html__( 'Relatest Post.', 'themesflat' )
            )
        );
    }
    ...
    function widget( $args, $instance ) {
		...
		<?php $i=1; if ( $flat_post->have_posts() ) : ?>
			<?php while ( $flat_post->have_posts() ) : $flat_post->the_post(); ?>
				<li class="clearfix">
                    ...
                    <div class="thumb">
                        ...
                        <?php 
                        if($i==1) {
                            the_post_thumbnail( 'themesflat-blog-1' );
                        } else 
                            the_post_thumbnail( 'themesflat-blog-2' );
                        ?>
                        </a>
                    </div>
                    <?php endif; ?>                       
                    <div class="text">  
                        <?php if ( $show_comment || $show_date) : ?>
                            <div class="post-meta">
                            <?php if ( $show_comment ) : ?>
                                <?php printf(
                                '<a class="meta-text" href="%s" title="%s" rel="author"> <i class="epictrips-icon-admin1"></i><span>'.esc_html__( 'By ', 'epictrips' ).'%s</span> </a>',
                                esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) )),
                                esc_attr( sprintf( esc_html__( 'View all posts by %s', 'epictrips' ), get_the_author() ) ),get_the_author()); ?>
                            <?php endif; ?>      
                            <?php if ( $show_date ) : ?>
                                <div class="post-date date updated"><i class="epictrips-icon-category"></i> <?php echo the_category( ', ' ); ?></div>
                            <?php endif; ?> 
                            </div> 
                        <?php endif; ?>                            
    ...
    function form( $instance ) {
      ...
      <p>
        ...
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_date' ) ); ?>"><?php esc_html_e( 'Show Category ?', 'themesflat' ); ?></label>
        </p>
        <p>
            ...
            <label for="<?php echo esc_attr( $this->get_field_id( 'show_comment' ) ); ?>"><?php esc_html_e( 'Show Author ?', 'themesflat' ); ?></label>
        </p>
    <?php
    }
}
