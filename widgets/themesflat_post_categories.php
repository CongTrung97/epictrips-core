<?php
class Themesflat_Post_Categories extends WP_Widget {
    /**
     ...
     * Constructor
     *
     * @return Themesflat_Post_Categories
     */
    function __construct() {
        ...
        parent::__construct(
            'widget_categories',
            esc_html__( 'Themesflat - Post Categories', 'themesflat' ),
            array(
                'classname'   => 'widget_categories',
                'description' => esc_html__( 'Post Categories.', 'themesflat' )
            )
        );
    }
    ...
    function widget( $args, $instance ) {
        ...
        extract( $instance );
        extract( $args );
        $classes[] = 'widget themesflat_widget_post_categories tf_widget_categories';
        echo wp_kses_post( $before_widget );
        ?>
        <div class="<?php echo esc_attr(implode(' ', $classes)) ;?>">
        <?php if ( !empty($title) ) echo wp_kses_post($before_title).esc_html($title).wp_kses_post($after_title);?>
            <ul>
                <?php if ( $custom_ids != '') {
                   $categories_ids = explode(',', $custom_ids);
                    foreach ($categories_ids as  $id) {
                        if (get_term_by('id',$id,'category') == true) {
                            ...
                            $count = esc_attr($term->count);
                            $count < 10 ? $count = '0'.$count : $count = $count;
                            @printf('<li class="cat-item cat-item-%1$d"><a href="%2$s">%3$s <span>test  %4$s</span> </a></li>',$id,esc_url(get_term_link($term)),esc_attr($term->name), $count);
                        }
                    }
                }
                else {
...
function themesflat_register_services_categories() {
    register_widget( 'Themesflat_Post_Categories' );
}